...
def delete1(host):
    seven_days_timestamp = time.time() - 7 * 24 * 60 * 60
    # thirty_days_timestamp = time.time() - 3 * 60 * 60
    seven_days_date = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(seven_days_timestamp))
    mg_query3 = {"HostIP":host, "collect_time":{"$lte":seven_days_timestamp}}
    col = db['activity']
    num = col.delete_many(mg_query3).deleted_count
    print(now, host, "delete resource before", seven_days_date, "total", num)
