...
from math import e
import operator
import process_classification.treePlotter as treePlotter
...
def calcShannonEnt(dataSet):
    ...
    for key in labelCounts:
        ...
        shannonEnt -= prob * log(prob, e)  # log base 2  计算信息熵
...
if __name__ == '__main__':
    fr = open('play_tennis.txt')
    lenses = [inst.strip().split(' ') for inst in fr.readlines()]  # 列表的列表，特征值和结果
    print(lenses)
    lensesLabels = ['outlook', 'temperature', 'humanity', 'windy']  # 特征
    lensesTree = createTree(lenses, lensesLabels)  # 字典，得到的决策树结果
    print(lensesTree)
    treePlotter.createPlot(lensesTree)  # 打印树
